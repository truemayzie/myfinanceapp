...
const telegram_1 = require("./_lib/telegram");
...
async function run(req, res) {
    ...
    const initData = (0, telegram_1.extractInitData)(req);
    if (!initData)
        return res.status(401).json({ error: 'missing auth' });
    const user = (0, telegram_1.verifyInitData)(initData, botToken);
    if (!user)
    ...
    if (req.method === 'POST') {
        ...
        const { error } = await supabase.from('finance_state').upsert({ telegram_id: tid, data: incoming, updated_at: new Date().toISOString() }, { onConflict: 'telegram_id' });
        if (error)
            return res.status(500).json({ error: error.message });
